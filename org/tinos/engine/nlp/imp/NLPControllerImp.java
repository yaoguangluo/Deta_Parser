...
public class NLPControllerImp implements NLPController{
	public int doSlangCheck(int countInputStringLength, List<String> output, String inputString,
		...
		if(wordsForest.containsKey(inputString)){
			...
			countInputStringLength = doPOSAndEMMCheck(countInputStringLength - StableData.INT_ONE
					, output, wordsForest, StableData.EMPTY_STRING + inputString.charAt(StableData.INT_ZERO) 
					+ inputString.charAt(StableData.INT_ONE) + inputString.charAt(StableData.INT_TWO)
					, prefixWord, posUtils);	
			return countInputStringLength;
		}
	}
	...
	public int doPOSAndEMMCheck(int countInputStringLength, List<String> outputList, Map<String, String> wordsForest
		...
		countInputStringLength = doSlangPartCheck(countInputStringLength - StableData.INT_ONE, outputList
				, strings[StableData.INT_ONE], wordsForest, prefixWord);
		return countInputStringLength;
	}
