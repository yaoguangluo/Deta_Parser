...
public class FHHMMListImp implements FHHMMList{
	...
	public Map<String, FHHMMNode> loopLoadForest(String cInputString) {
		for(int i = DataString.INT_ZERO; i < cInputString.length(); i++) {
			if(linkedHashMap.containsKey(DataString.EMPTY_STRING + cInputString.charAt(i))) {
				FHHMMNode fFHMMNode = linkedHashMap.get(DataString.EMPTY_STRING 
						+ cInputString.charAt(i));
	...
	public Map<String, FHHMMNode> doNeroPostCognitive(FHHMMNode fFHMMNode, String cInputString, int i) {
		if(fFHMMNode.getNext() != null) {
			...
			if(i + DataString.INT_ONE < cInputString.length()) {
				innerLinkedHashMap.put(DataString.EMPTY_STRING + cInputString.charAt(i + DataString
						.INT_ONE), DataString.INT_ONE);
			} 
	...
	public Map<String, FHHMMNode> doCheckAndRunNeroPostFix(FHHMMNode fFHMMNode, String cInputString
			, int i) {
		if(!fFHMMNode.getNext().containsKey(DataString.EMPTY_STRING + cInputString.charAt(i 
			...
			innerMap.put(DataString.EMPTY_STRING + cInputString.charAt(i + DataString.INT_ONE)
			, DataString.INT_ONE);
