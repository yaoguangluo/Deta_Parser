...
#### 4 基于 《线性语料库》+《森林网络字典索引》+《欧基里德神经网络加权搜索》+《2分法位运算字符集处理》+《索引树深度正向隐马可夫匹配》+《逆向马可夫词性修正策略》 的《VPC架构》 中文分词包。
...
#### 5.1 每秒高达900万词语的中文词性索引速度。（Part Of Speech, POS）
#### 5.2 多大8000+的中文语料库，有效的辨别新词。
#### 6 大小55Kb。
...
应用方法：本人用FNLP函数将文章中的词语进行分词并词性标注，得到的标注如果在我的词库里面没有出现，于是扩充在我的词库。特此声明。
