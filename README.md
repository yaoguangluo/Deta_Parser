...
#### 4 扩展词语简单：基于 《线性语料库》。
#### 5 查询词语方便：基于 《森林网络字典索引》。
#### 7 搜索词语加速：基于 《2分法欧几里德距离位运算哈希散列存储字符集处理索引搜索数据字典》。
#### 8 匹配词语精准：基于 《决策树深度 NLP 正向隐马可夫匹配》。
#### 9 病句分析完善：基于 《逆向马可夫词性 POS 修正策略》。
#### 10 速度：每秒高达1050万中文简体字准确分词。 因为通过国际SONAR最高认证，牺牲了程序执行时间十分之三的速度效率（自行修改去掉sonar认知模式可达1600万字分词每秒，性能比应该是世界第二，世界第一赠给高斯林先生，因为我用的是java，没办法）。
##### 10.1 速度每秒高达900万词语的中文词性索引。（Part Of Speech, POS），
##### 10.2 机制为分词和词性分析可拆分使用。采用一次实例，多并发执行思想。
#### 11 词库：多达12700+的中文语料库精确简体中文词汇，有效的辨别新词。
#### 12 大小：55Kb。
#### 13 多核模式：可以自己写 parallelStream() 函数去实现，jdk8以上已经支持, CogsBinaryForestAnalyzer 支持海量多核多线程并发安全 。
#### 14 安全：VPC架构采用纯虚函数做反向映射跳过IOC，效率增加，线程安全高度严格保障。
## 使用方法：
#### 1 支持 java JDK 8 以上。
#### 2 字符集UTF-8
